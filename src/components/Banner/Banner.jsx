...
import { deleteBanner, fetchBanner } from "../../stores/actionCreator";
...
export default function Banner() {
  ...
  const filteredBanner = Banner.filter(el =>
    el.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
    el.details.toLowerCase().includes(searchQuery.toLowerCase()) ||
    el.link.toLowerCase().includes(searchQuery.toLowerCase())
  );
            ...
            <tbody>
              {filteredBanner.map((el, i) => (
                <tr key={i} className="text-sm">
                      ...
                      <div className="rounded-md w-20 lg:w-[200px] lg:h-28 h-20 overflow-hidden group">
                        ...
                        <div className="absolute inset-0 rounded-md w-20 lg:w-[200px] lg:h-28 h-20   bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                          <span
                            onClick={() =>
                              handleImageClick(
                                `https://api-v1.yestech.id/${el.imageBanner}`
                              )
                            }
                  ...
                  <td className="px-2 py-1">{el.details}</td>
